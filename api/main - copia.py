...
def process_results_1(results):
    ...
    for row in results:
        ...
        # Verificar si valor_desde es 'Coleccion'
        if valor_desde == 'Coleccion':
            continue  # Skip this iteration if valor_desde is 'Coleccion'
            
        # Procesar solo si valor_desde no es 'Coleccion'
        ...
        if mongodb_query:  # Solo agregar si hay un query válido
            processed_data[idboni]["QUERYMONGODB"]["$and"].append(mongodb_query)
    
    return processed_data


# Function to process results 2
...
# Function to process the 'Obsequios' table
...
# Function to merge results from SQL queries and MongoDB data
...
# API endpoint for processing promotions based on the codigo
@app.route('/promos', methods=['POST'])
def get_promos():
    try:
        # Obtener el parámetro "codigo" desde el cuerpo de la solicitud
        ...
        # Ejecutar la consulta para obtener el IDBoni usando el parámetro "codigo"
        #idboni_list = execute_query(sql_connection, SQL_QUERY_IDBONI, ("'%' + codigo + '%'",))
        # Corrección:
        idboni_list = execute_query(sql_connection, SQL_QUERY_IDBONI, (f"%{codigo}%",))
        
        if not idboni_list:
            return jsonify({"message": "No se encontraron promociones para el código proporcionado."}), 404

        # Procesar los resultados de las consultas SQL y los obsequios
        merged_data = []
