...
WHERE 
    D.PKID = 600078762 and ddd.TieneReglaExclusion=1
    	and 
	(    CASE 
        WHEN CHARINDEX('/', RTRIM(DDD.RutaCaracteristicaEstructural)) > 0 
        THEN SUBSTRING(RTRIM(DDD.RutaCaracteristicaEstructural), 
...
SQL_QUERY_2 = """
    ...
    WHERE 
        D.PKID = 600078762 AND DDD.TieneReglaExclusion = 0
...
where d.pkid = 600078762
"""

#SQL_QUERY_IDBONI = """
#SELECT pkid AS IDBoni,Descripcion,codigo FROM DefinicionBonificacion
#WHERE (Activo=1 AND (Codigo LIKE '%VAN%2024-11%' ) AND Historico=0 ) 
#"""

#SQL_QUERY_IDBONI = """
#SELECT pkid AS IDBoni,Descripcion,codigo FROM DefinicionBonificacion
#WHERE (Activo=0 AND (Codigo LIKE '%RUT%2025-01%' ) AND Historico=0 )  
#"""

SQL_QUERY_IDBONI = """
SELECT pkid AS IDBoni FROM DefinicionBonificacion
WHERE (Activo=1 and CODIGO like '%rut%cor%' AND Historico=0)  
"""
...
def main():
    ...
    try:
        ...
        # Crear índice para timestamps si no existe
        ...
        for idboni_row in idboni_list:
            ...
            print(f"Processing IDBoni: {idboni}")
            
            # Modificar las consultas SQL para usar el IDBoni actual
            current_sql_query_1 = SQL_QUERY_1.replace("D.PKID = 600078762", f"D.PKID = {idboni}")
            current_sql_query_2 = SQL_QUERY_2.replace("D.PKID = 600078762", f"D.PKID = {idboni}")
            current_sql_query_3 = SQL_QUERY_3.replace("d.pkid = 600078762", f"d.pkid = {idboni}")
