...
WHERE 
    D.PKID = 72830 and ddd.TieneReglaExclusion=1
...
SQL_QUERY_2 = """
    ...
    WHERE 
        D.PKID = 72830 AND DDD.TieneReglaExclusion = 0
)
SELECT *
FROM CTE
...
#WHERE (Activo=1 AND (Codigo LIKE '%PYG%' AND Codigo NOT LIKE '%MD%' AND Codigo NOT LIKE '%MY%' and Codigo LIKE '%DAVINCI%'))
#"""


SQL_QUERY_IDBONI = """
SELECT pkid AS IDBoni,rtrim(Codigo) as CodigoPromocion,rtrim(Descripcion) as Descripcion FROM DefinicionDescuento2
WHERE (Activo=1 AND (Codigo LIKE '%ENE25 PYG VN-VC-VE-NC RT DAVINCI 111'))
"""
...
def main():
    ...
    try:
        ...
        for idboni_row in idboni_list:
            ...
            current_sql_query_1 = SQL_QUERY_1.replace("D.PKID = 72830", f"D.PKID = {idboni}")
            current_sql_query_2 = SQL_QUERY_2.replace("D.PKID = 72830", f"D.PKID = {idboni}")
            current_sql_query_3 = SQL_QUERY_3.replace("d.pkid = 72830", f"d.pkid = {idboni}")
