...
WHERE 
    D.PKID = 600865766 and ddd.TieneReglaExclusion=1
"""
...
WITH CTE AS (
    ...
    WHERE 
        D.PKID = 600865766 AND DDD.TieneReglaExclusion = 0
)
SELECT *
FROM CTE
...
where d.pkid = 600865766
"""

#SQL_QUERY_IDBONI = """
#SELECT pkid AS IDBoni FROM DefinicionBonificacion
#WHERE (Activo=1 AND (Codigo LIKE '%PYG%' AND Codigo NOT LIKE '%MD%' AND Codigo NOT LIKE '%MY%') AND Historico=0)
#"""


SQL_QUERY_IDBONI = """
SELECT pkid AS IDBoni FROM DefinicionBonificacion
WHERE (Activo=1 AND (Codigo LIKE 'JUN25 PYG NC RT DAVINCI 128%') AND Historico=0)
"""
...
def main():
    ...
    try:
        ...
        for idboni_row in idboni_list:
            ...
            current_sql_query_1 = SQL_QUERY_1.replace("D.PKID = 600865766", f"D.PKID = {idboni}")
            current_sql_query_2 = SQL_QUERY_2.replace("D.PKID = 600865766", f"D.PKID = {idboni}")
            current_sql_query_3 = SQL_QUERY_3.replace("d.pkid = 600865766", f"d.pkid = {idboni}")
